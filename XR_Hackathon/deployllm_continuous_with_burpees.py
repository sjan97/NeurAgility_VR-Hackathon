...
# ------------------- Feedback / CSV -------------------
def send_llm_feedback_with_metrics(metrics, llm_feedback):
    payload = {
        "type": "metrics",
        "total_reps": metrics["total_reps"],
        "avg_knee_angle": metrics.get("avg_knee_angle"),
        "avg_elbow_angle": metrics.get("avg_elbow_angle"),
        "min_knee_angle": metrics.get("min_knee_angle"),
        "min_elbow_angle": metrics.get("min_elbow_angle"),
        "llm_feedback": llm_feedback
    }
    data = json.dumps(payload).encode("utf-8")
    sock.sendto(data, (UDP_IP, UDP_PORT))
    print("Sent metrics+LLM feedback to Unity:", payload)
...
def append_workout_to_csv(metrics, exercise_type, rule_feedback):
    # Default header for most exercises
    header = ["timestamp", "exercise_type", "total_reps",
              "avg_knee_angle", "min_knee_angle",
              "avg_elbow_angle", "min_elbow_angle",
              "avg_shoulder_angle", "min_shoulder_angle",
              "rule_feedback_summary"]

    feedback_summary = ";".join(rule_feedback)

    # Build row depending on exercise type
    if exercise_type == "standing_burpee":
        row = [datetime.now().isoformat(), exercise_type, metrics["total_reps"],
               None, None, None, None,
               metrics.get("avg_shoulder_angle"), metrics.get("min_shoulder_angle"),
               feedback_summary]
    else:
        row = [datetime.now().isoformat(), exercise_type, metrics["total_reps"],
               metrics.get("avg_knee_angle"), metrics.get("min_knee_angle"),
...
3) One particular column/metric the user has been doing very well over the course of history and how it has improved percentise wise. Provide extremely concise advice in ~20 words.
"""
    try:
...
#         if min_knee > 120: feedback_tags.append("shallow_squat")
#         elif min_knee < 80: feedback_tags.append("deep_squat")
#         else: feedback_tags.append("good_depth")
#         if avg_knee < 90: feedback_tags.append("strong_squat")
#         elif avg_knee > 110: feedback_tags.append("needs_more_control")
#     elif ex_type == "pushup":
#         elbow_angles = angles["elbow"]
#         min_elbow = min(elbow_angles)
#         if min_elbow > 110: feedback_tags.append("lower_body_more")
#         else: feedback_tags.append("good_pushup_form")
#     else:
...
def basic_feedback(ex_type, angles):
    ...
    if ex_type == "squat":
        ...
        # original burpee (floor + jump)
        elbow_angles = angles["elbow"]
        knee_angles = angles["knee"]
        shoulder_angles = angles["shoulder"]
        if min(knee_angles) > 120: feedback_tags.append("not_low_enough")
        else: feedback_tags.append("good_depth")
        if min(elbow_angles) > 110: feedback_tags.append("arms_not_low_enough")
        else: feedback_tags.append("good_pushup_position")
        if max(shoulder_angles) < 150: feedback_tags.append("needs_more_jump")
        else: feedback_tags.append("good_jump")

    elif ex_type == "standing_burpee":  # <-- new exercise
        shoulder_angles = angles["shoulder"]
        max_shoulder = max(shoulder_angles) if shoulder_angles else None
        min_shoulder = min(shoulder_angles) if shoulder_angles else None

        if max_shoulder and max_shoulder < 150:
            feedback_tags.append("needs_more_raise")
        else:
            feedback_tags.append("good_raise")

        if min_shoulder and min_shoulder > 100:
            feedback_tags.append("not_lowering_enough")
        else:
            feedback_tags.append("good_lowering")
...
def process_video(video_path, exercise_type="squat", output_path="annotated_3d.mp4"):
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while not exit_flag:
            ret, frame = cap.read()
            if not ret:
                if video_path.lower().startswith("http"):
                    continue  # keep trying for URL streams
                else:
            ...
            if results.pose_landmarks:
                ...
                if exercise_type == "squat":
                    ...
                    if elbow_angle < 90 and stage == "up":
                        stage = "down"
                        reps += 1
                elif exercise_type == "standing_burpee":  # <-- new exercise
                    # Count rep when shoulders go up (>150) then down (<100)
                    if shoulder_angle > 150 and stage != "up":
                        stage = "up"
                    if shoulder_angle < 100 and stage == "up":
                        stage = "down"
                        reps += 1
                ...
                cv2.putText(image, f"Elbow: {int(elbow_angle)} Knee: {int(knee_angle)} Shoulder: {int(shoulder_angle)}",
                (30, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,255,255), 2)
            ...
            if time.time() - last_publish_time >= 5:
                ...
                send_llm_feedback_with_metrics(metrics, llm_fb)
                last_publish_time = time.time()

    # ---------- On exit: save CSV, plots, feedback ----------
    # metrics = {
    #     "total_reps": reps,
    #     "avg_knee_angle": round(mean(knee_angles), 1) if knee_angles else None,
    #     "avg_elbow_angle": round(mean(elbow_angles), 1) if elbow_angles else None,
    ...
    if exercise_type == "standing_burpee":
        metrics = {
            "total_reps": reps,
            "avg_shoulder_angle": round(mean(shoulder_angles), 1) if shoulder_angles else None,
            "min_shoulder_angle": round(min(shoulder_angles), 1) if shoulder_angles else None
        }
    else:
        metrics = {
            "total_reps": reps,
    ...
    with open("feedback.txt", "w") as f:
        ...
        f.write(str(metrics) + "\n\n")
        f.write("FEEDBACK:\n" + llm_fb)
    print("CSV and feedback saved on exit.")
    cap.release()
    out.release()
...
if __name__ == "__main__":
    VIDEO_PATH = "d3.mp4"  # or your URL
    # EXERCISE_TYPE = "squat"
    EXERCISE_TYPE = "standing_burpee"
    OUTPUT_PATH = "annotated_output_ananya2.mp4"
